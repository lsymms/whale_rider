...
Design and implementation for a locally hosted stock and options flow alerting webapp, with Telegram delivery, screenshot-assisted position updates, and exposure-aware AI trade suggestions.

**Status:** local MVP implementation in progress. Examples and prototype data are synthetic unless a command explicitly says it uses the configured live provider keys. Prepared September 14, 2026 for a user who already has a Webull OpenAPI key.
...
12. [Live market-data testing](docs/live-market-data-testing.md) - how to verify Nasdaq Basic and OPRA real-time access from the app and CLI.
...
No production credentials, screenshots, account data, backend, or live API connection are committed. This is a local Git repository with a private GitHub remote.
